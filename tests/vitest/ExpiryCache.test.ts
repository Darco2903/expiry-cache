...
describe("ExpiryCache", () => {
    it("constructor sets expired when data is null", () => {
        const cache = new ExpiryCache<number, () => number>(null, () => 1, 1000);
    ...
    it("invalidate clears data and marks expired", () => {
        const cache = new ExpiryCache(42, () => 1, 1000);
    ...
    it("refresh updates data and expiration", async () => {
        ...
        const cache = new ExpiryCache<number, typeof cb>(null, cb, 50);
        await cache.refresh();
    ...
    it("concurrent refresh calls use single-flight", async () => {
        ...
        const cache = new ExpiryCache<number, typeof cb>(null, cb, 100);
    ...
    it("refresh preserves old data on error and propagates error", async () => {
        ...
        const cache = new ExpiryCache<number, typeof goodCb>(null, goodCb, 10);
        await cache.refresh();
